...
int main(int argc, char **argv) {
  ...
  printf(
      "test-producer: dmabuf fd=%d %ux%u stride=%u modifier=0x%llx slot=%u\n",
      dmabuf_fd, TEST_WIDTH, TEST_HEIGHT, stride, (unsigned long long)modifier,
      slot);
  ...
  if (connect(sock_fd, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
    fprintf(
        stderr,
        "test-producer: connect(%s) failed: %s (is the portal listening?)\n",
  ...
  char buf_header[256];
  snprintf(buf_header, sizeof(buf_header), "BUF %u %u %u 0 %u %llu", slot,
           (unsigned)TEST_WIDTH, (unsigned)TEST_HEIGHT, stride,
