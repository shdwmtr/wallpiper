...
__attribute__((constructor)) static void wp_register_guardian(void)
{
    ...
    if (pidfd < 0) {
        wp_log("guardian pid %d already gone, self-terminating pid=%d", (int)guardian_pid, (int)getpid());
        raise(SIGKILL);
        return;
    }

    pthread_t thread;
    if (pthread_create(&thread, NULL, wp_guardian_watch, (void*)(intptr_t)pidfd) != 0) {
    ...
    wp_log("watching guardian pid=%d from pid=%d", (int)guardian_pid, (int)getpid());
}
