...
static gboolean on_ctl_socket_connectable(gint fd, GIOCondition condition, gpointer user_data)
{
    ...
    if (g_strcmp0(line, "GEOMETRY") == 0) {
        ...
        wallpiper_actor_stacking_dump_children(state->parent, "debug-on");
        {
            ...
            for (ClutterActor* child = clutter_actor_get_first_child(state->parent); child != NULL; child = clutter_actor_get_next_sibling(child), idx++) {
                if (child == state->display_actor) {
                    g_message("wallpiper-gnome: [debug-on] our display_actor is at index %u", idx);
                    found = TRUE;
                    break;
                }
            }
            if (!found)
                g_message("wallpiper-gnome: [debug-on] our display_actor NOT FOUND among "
        ...
        MetaCursorTracker* tracker = meta_backend_get_cursor_tracker(state->backend);
        graphene_point_t point = GRAPHENE_POINT_INIT(0.f, 0.f);
        if (tracker) meta_cursor_tracker_get_pointer(tracker, &point, NULL);
        response = g_strdup_printf("CURSOR_POS %d %d\n", (int)point.x, (int)point.y);
    } else {
...
gboolean wallpiper_ctl_listener_start(WallpiperPortalState* state, GError** error)
{
    ...
    if (ctl_fd < 0) {
        g_set_error(error, WALLPIPER_ERROR, 0, "ctl socket() failed: %s", g_strerror(errno));
        return FALSE;
    }
    ...
    if (bind(ctl_fd, (struct sockaddr*)&addr, sizeof(addr)) != 0 || listen(ctl_fd, 4) != 0) {
        g_set_error(error, WALLPIPER_ERROR, 0, "bind/listen(%s) failed: %s", WALLPIPER_CTL_SOCKET_PATH, g_strerror(errno));
        close(ctl_fd);
        return FALSE;
    }

    state->ctl_socket_fd = ctl_fd;
    state->ctl_source_id = g_unix_fd_add(ctl_fd, G_IO_IN, on_ctl_socket_connectable, state);
