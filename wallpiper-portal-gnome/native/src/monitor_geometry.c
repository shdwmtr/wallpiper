...
{
    WallpiperMonitorGeometry fallback = { 0, 0, 1920, 1080, 1920, 1080 };

    MetaMonitorManager* manager = meta_backend_get_monitor_manager(backend);
    if (!manager) return fallback;

    MetaLogicalMonitor* logical_monitor = meta_monitor_manager_get_primary_logical_monitor(manager);
    if (!logical_monitor) {
        GList* logical_monitors = meta_monitor_manager_get_logical_monitors(manager);
        if (!logical_monitors) return fallback;
...
{
    return g_strdup_printf("{\"x\":%d,\"y\":%d,\"width\":%u,\"height\":%u,"
                           "\"logical_width\":%u,\"logical_height\":%u}",
