...
gboolean wallpiper_place_test_actor(GObject* backend_obj, GObject* parent_obj, GObject* below_sibling_obj, int x, int y, int width, int height, GError** error)
{
    if (!META_IS_BACKEND(backend_obj)) {
        g_set_error(error, WALLPIPER_ERROR, 0, "backend argument is not a MetaBackend");
        return FALSE;
    }

    if (!CLUTTER_IS_ACTOR(parent_obj)) {
        g_set_error(error, WALLPIPER_ERROR, 0, "parent argument is not a ClutterActor");
        return FALSE;
    }

    if (below_sibling_obj && !CLUTTER_IS_ACTOR(below_sibling_obj)) {
        g_set_error(error, WALLPIPER_ERROR, 0, "below_sibling argument is not a ClutterActor");
        return FALSE;
    }

    ClutterActor* parent = CLUTTER_ACTOR(parent_obj);
    ClutterActor* below_sibling = below_sibling_obj ? CLUTTER_ACTOR(below_sibling_obj) : NULL;
    ...
    g_message("wallpiper-gnome: got cogl_context=%p egl_display=%p", (void*)cogl_context, (void*)egl_display);

    if (!egl_display) {
        g_set_error(error, WALLPIPER_ERROR, 0, "could not get Mutter's EGLDisplay from Cogl");
        return FALSE;
    }
    ...
    if (!gbm_dev) {
        g_set_error(error, WALLPIPER_ERROR, 0, "could not open a DRM render node for GBM");
        return FALSE;
    }
    ...
    if (!bo) {
        ...
        bo = gbm_bo_create(gbm_dev, TEST_WIDTH, TEST_HEIGHT, GBM_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
    }
    if (!bo) {
        int saved_errno = errno;
        g_message("wallpiper-gnome: gbm_bo_create(RENDERING) failed: %s, retrying "
                  ...
                  g_strerror(saved_errno));
        bo = gbm_bo_create(gbm_dev, TEST_WIDTH, TEST_HEIGHT, GBM_FORMAT_XRGB8888, GBM_BO_USE_SCANOUT | GBM_BO_USE_RENDERING);
    }
    if (!bo) {
        int saved_errno = errno;
        g_set_error(error, WALLPIPER_ERROR, 0, "gbm_bo_create failed in every flag combination tried: %s", g_strerror(saved_errno));
    ...
    g_message("wallpiper-gnome: dmabuf fd=%d stride=%u offset=%u modifier=0x%llx", dmabuf_fd, stride, offset, (unsigned long long)modifier);

    CoglTexture* texture = wallpiper_egl_import_dmabuf(cogl_context, egl_display, dmabuf_fd, TEST_WIDTH, TEST_HEIGHT, stride, offset, modifier, error);
