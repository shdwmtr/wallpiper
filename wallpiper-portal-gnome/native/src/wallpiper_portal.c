...
gboolean wallpiper_portal_start(GObject* backend_obj, GObject* parent_obj, GError** error)
{
    if (portal_state) {
    ...
    g_message("wallpiper-gnome: detected monitor geometry %ux%u at (%d,%d), "
              "logical %ux%u",
              geometry.width, geometry.height, geometry.x, geometry.y, geometry.logical_width, geometry.logical_height);
    ...
    ClutterActor* background_actor = wallpiper_actor_stacking_dump_children(parent, "enable");
    if (background_actor)
    ...
    state->meta_display = meta_context ? meta_context_get_display(meta_context) : NULL;
...
void wallpiper_portal_stop(void)
{
    ...
    if (portal_state->display_actor) clutter_actor_destroy(portal_state->display_actor);
