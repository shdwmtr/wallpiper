...
class EglDmabufImporter
{
  public:
    ...
    std::optional<Import> importDmabuf(int width, int height, uint32_t stride, uint64_t modifier, int fd) const;
    void destroyImport(const Import& import) const;
    void refreshBinding(const Import& import) const;

    std::optional<EGLImageKHR> createImageOnly(int width, int height, uint32_t stride, uint64_t modifier, int fd) const;
    void destroyEglImage(EGLImageKHR image) const;
    bool waitForSyncFd(int syncFd) const;

    QSGTexture* wrapExternalOes(QQuickWindow* window, const Import& import, const QSize& size) const;
    QSGTexture* wrapTexture2D(QQuickWindow* window, unsigned int texture, const QSize& size) const;

  private:
    using GlEglImageTargetTexture2DOesFn = void (*)(unsigned int target, void* image);
    using EglCreateSyncKHRFn = EGLSyncKHR (*)(EGLDisplay, EGLenum, const EGLint*);
    using EglDestroySyncKHRFn = EGLBoolean (*)(EGLDisplay, EGLSyncKHR);
    using EglClientWaitSyncKHRFn = EGLint (*)(EGLDisplay, EGLSyncKHR, EGLint, EGLTimeKHR);
