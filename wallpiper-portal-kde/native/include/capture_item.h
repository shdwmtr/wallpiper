...
class WallpaperCaptureItem : public QQuickItem
{
  ...
  public:
    ...
    void stageBuf(quint32 slot, quint32 width, quint32 height, quint32 stride, quint64 modifier, int fd, int syncFd);
    ...
    double peakFrameMs() const
    {
        return m_peakFrameMs;
    }
  ...
  private:
    ...
    void recordCapture();
    void recordDisplay();
    static void pruneWindow(std::deque<qint64>& timestamps, qint64 nowMs, qint64 windowMs);
    static int countWithin(const std::deque<qint64>& timestamps, qint64 nowMs, qint64 windowMs);
