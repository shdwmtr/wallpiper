...
CtlListener::CtlListener(QString portalName, QObject* parent) : QObject(parent), m_portalName(std::move(portalName))
{
}

CtlListener::~CtlListener()
{
    stop();
}
...
{
    const std::string path = WallpiperProtocol::ctlSocketPath(m_portalName).toStdString();
    int listenFd = WallpiperProtocol::bindUnixStreamListener(path);
    if (listenFd < 0) {
        qWarning() << "[ctl] failed to bind" << QString::fromStdString(path) << ":" << std::strerror(errno);
        m_running = false;
        return;
    }
    m_listenFd = listenFd;
...
void CtlListener::handleConnection(int clientFd)
{
    ...
    auto writeResponse = [clientFd](const WallpiperProtocol::CtlResponse& response)
    {
        std::string encoded = WallpiperProtocol::encodeCtlResponse(response);
        size_t written = 0;
        while (written < encoded.size()) {
            ssize_t n = ::write(clientFd, encoded.data() + written, encoded.size() - written);
            if (n <= 0) {
                return;
            }
    ...
    if (*request == WallpiperProtocol::CtlRequest::CursorPos) {
        WallpiperProtocol::CtlResponse response = WallpiperProtocol::CtlResponseErr{ "cursor position unavailable" };
        if (m_cursorPosProvider) {
            if (auto pos = m_cursorPosProvider()) {
                response = WallpiperProtocol::CtlResponseCursorPos{ pos->first, pos->second };
            }
        }
    ...
    WallpiperProtocol::CtlResponse response = WallpiperProtocol::CtlResponseErr{ "listener shutting down" };
    switch (*request) {
