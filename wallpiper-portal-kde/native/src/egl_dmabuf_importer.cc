...
{
    return m_display != EGL_NO_DISPLAY && m_glEGLImageTargetTexture2DOES != nullptr;
}
...
bool EglDmabufImporter::ensureBound(QQuickWindow* window)
{
    ...
    auto* ctx = reinterpret_cast<QOpenGLContext*>(rif->getResource(window, QSGRendererInterface::OpenGLContextResource));
    if (!ctx) {
    ...
    m_display = display;
    m_glEGLImageTargetTexture2DOES = reinterpret_cast<GlEglImageTargetTexture2DOesFn>(proc);
    m_eglCreateSyncKHR = reinterpret_cast<EglCreateSyncKHRFn>(eglGetProcAddress("eglCreateSyncKHR"));
    m_eglDestroySyncKHR = reinterpret_cast<EglDestroySyncKHRFn>(eglGetProcAddress("eglDestroySyncKHR"));
    m_eglClientWaitSyncKHR = reinterpret_cast<EglClientWaitSyncKHRFn>(eglGetProcAddress("eglClientWaitSyncKHR"));
...
std::optional<EglDmabufImporter::Import> EglDmabufImporter::importDmabuf(int width, int height, uint32_t stride, uint64_t modifier, int fd) const
{
    ...
    EGLImage image = eglCreateImage(m_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, nullptr, attribs);
    if (image == EGL_NO_IMAGE) {
        qWarning() << "[egl] eglCreateImage failed, EGL error" << Qt::hex << eglGetError();
        return std::nullopt;
    }
    ...
    gl->glTexParameteri(kGlTextureExternalOes, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    gl->glTexParameteri(kGlTextureExternalOes, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
std::optional<EGLImageKHR> EglDmabufImporter::createImageOnly(int width, int height, uint32_t stride, uint64_t modifier, int fd) const
{
    const FdCloser fdCloser{ fd };
    ...
    EGLImage image = eglCreateImage(m_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, nullptr, attribs);
    if (image == EGL_NO_IMAGE) {
        qWarning() << "[egl] eglCreateImage (refresh) failed, EGL error" << Qt::hex << eglGetError();
        return std::nullopt;
    }
    return image;
}
...
bool EglDmabufImporter::waitForSyncFd(int syncFd) const
{
    ...
    if (!isBound() || !m_eglCreateSyncKHR || !m_eglClientWaitSyncKHR || !m_eglDestroySyncKHR) {
        ::close(syncFd);
        return false;
    }
    ...
    EGLSyncKHR sync = m_eglCreateSyncKHR(m_display, EGL_SYNC_NATIVE_FENCE_ANDROID, attribs);
    if (sync == EGL_NO_SYNC_KHR) {
        qWarning() << "[egl] eglCreateSyncKHR failed, EGL error" << Qt::hex << eglGetError();
        ::close(syncFd);
        return false;
    }

    constexpr EGLTimeKHR kWaitTimeoutNs = 200'000'000;
    EGLint result = m_eglClientWaitSyncKHR(m_display, sync, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, kWaitTimeoutNs);
    m_eglDestroySyncKHR(m_display, sync);

    if (result != EGL_CONDITION_SATISFIED_KHR) {
        qWarning() << "[egl] eglClientWaitSyncKHR did not signal in time, result" << Qt::hex << result;
        return false;
    }
    return true;
}
...
QSGTexture* EglDmabufImporter::wrapExternalOes(QQuickWindow* window, const Import& import, const QSize& size) const
{
    return QNativeInterface::QSGOpenGLTexture::fromNativeExternalOES(import.texture, window, size);
}

QSGTexture* EglDmabufImporter::wrapTexture2D(QQuickWindow* window, unsigned int texture, const QSize& size) const
{
