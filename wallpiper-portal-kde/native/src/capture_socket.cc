...
{
    int sockFd = WallpiperProtocol::bindUnixDgram(WallpiperProtocol::kCaptureSocketPath);
    if (sockFd < 0) {
        qWarning() << "[capture] failed to bind" << WallpiperProtocol::kCaptureSocketPath;
        m_running = false;
        return;
    }
    m_sockFd = sockFd;
    qInfo() << "[capture] listening on" << WallpiperProtocol::kCaptureSocketPath << "(dgram)";
    ...
    while (m_running.load()) {
        ...
        if (!event) {
            qWarning() << "[capture] unrecognized or malformed message:" << QString::fromStdString(received->header);
        ...
        if (const auto* buf = std::get_if<WallpiperProtocol::BufEvent>(&*event)) {
            emit bufReceived(buf->slot, buf->width, buf->height, buf->stride, buf->modifier, buf->fd, buf->syncFd);
        } else if (const auto* frame = std::get_if<WallpiperProtocol::FrameEvent>(&*event)) {
            emit frameReceived(frame->slot, frame->syncFd);
        } else if (const auto* shm = std::get_if<WallpiperProtocol::ShmEvent>(&*event)) {
