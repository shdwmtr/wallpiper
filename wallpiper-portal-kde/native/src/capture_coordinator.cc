...
CaptureCoordinator::CaptureCoordinator(QObject* parent)
    : QObject(parent), m_captureSocket(new CaptureSocket(this)), m_ctlListener(new CtlListener(QStringLiteral("kde"), this)), m_cursorTimer(new QTimer(this))
{
    connect(m_captureSocket, &CaptureSocket::bufReceived, this, [this](quint32 slot, quint32 width, quint32 height, quint32 stride, quint64 modifier, int fd, int syncFd)
    {
        if (m_active) {
            m_active->stageBuf(slot, width, height, stride, modifier, fd, syncFd);
        } else {
            ::close(fd);
            if (syncFd >= 0) {
                ::close(syncFd);
    ...
    connect(m_captureSocket, &CaptureSocket::frameReceived, this, [this](quint32 slot, int syncFd)
    {
        if (m_active) {
    ...
    connect(m_captureSocket, &CaptureSocket::shmReceived, this, [this](quint32 width, quint32 height, quint32 stride, int fd)
    {
        if (m_active) {
    ...
    m_ctlListener->setCursorPosProvider([this]() -> std::optional<std::pair<int32_t, int32_t>>
    {
        return std::make_pair(m_cursorX.load(), m_cursorY.load());
    });
    m_ctlListener->setGeometryProvider([this]() -> std::optional<WallpiperProtocol::MonitorGeometry>
    {
        std::optional<WallpiperProtocol::MonitorGeometry> result;
        QMetaObject::invokeMethod(this, [this, &result]()
        {
            result = geometryFromActiveItem();
...
{
    if (!item || std::find(m_items.begin(), m_items.end(), item) != m_items.end()) {
        return;
    }
...
void CaptureCoordinator::reevaluateActiveItem()
{
    ...
    if (m_active) {
        qInfo() << "[coordinator] active wallpaper item is now on screen"
                << (m_active->window() && m_active->window()->screen() ? m_active->window()->screen()->name() : QStringLiteral("<none>"));
    }
}
...
std::optional<WallpiperProtocol::MonitorGeometry> CaptureCoordinator::geometryFromActiveItem() const
{
    if (!m_active) {
        return std::nullopt;
    }
