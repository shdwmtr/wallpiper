...
{
    return x == other.x && y == other.y && width == other.width && height == other.height && logicalWidth == other.logicalWidth && logicalHeight == other.logicalHeight;
}
...
QJsonObject toJson(const MonitorGeometry& geometry)
{
    ...
    object[QStringLiteral("logical_width")] = static_cast<qint64>(geometry.logicalWidth);
    object[QStringLiteral("logical_height")] = static_cast<qint64>(geometry.logicalHeight);
    return object;
}

std::optional<MonitorGeometry> geometryFromJson(const QJsonObject& object)
{
    if (!object.contains(QStringLiteral("x")) || !object.contains(QStringLiteral("y")) || !object.contains(QStringLiteral("width")) || !object.contains(QStringLiteral("height")) ||
        !object.contains(QStringLiteral("logical_width")) || !object.contains(QStringLiteral("logical_height"))) {
        return std::nullopt;
    }
    MonitorGeometry geometry;
    geometry.x = object[QStringLiteral("x")].toInt();
    geometry.y = object[QStringLiteral("y")].toInt();
    geometry.width = static_cast<uint32_t>(object[QStringLiteral("width")].toInt());
    geometry.height = static_cast<uint32_t>(object[QStringLiteral("height")].toInt());
    geometry.logicalWidth = static_cast<uint32_t>(object[QStringLiteral("logical_width")].toInt());
    geometry.logicalHeight = static_cast<uint32_t>(object[QStringLiteral("logical_height")].toInt());
    return geometry;
}
...
std::optional<SocketEvent> parseEvent(const std::string& header, const std::vector<int>& fds)
{
...
std::optional<RecvResult> recvMsg(int sockFd)
{
    ...
    while (!result.header.empty() && std::isspace(static_cast<unsigned char>(result.header.back()))) {
        result.header.pop_back();
    }
    size_t start = 0;
    while (start < result.header.size() && std::isspace(static_cast<unsigned char>(result.header[start]))) {
        ++start;
    }
...
int bindUnixDgram(const std::string& path)
{
    ...
    auto addrLen = static_cast<socklen_t>(offsetof(sockaddr_un, sun_path) + path.size() + 1);
...
std::optional<CtlRequest> parseCtlRequest(const std::string& line)
{
    ...
    if (trimmed == "GEOMETRY") return CtlRequest::Geometry;
    if (trimmed == "DETACH") return CtlRequest::Detach;
    if (trimmed == "DEBUG_ON") return CtlRequest::DebugOn;
    if (trimmed == "DEBUG_OFF") return CtlRequest::DebugOff;
    if (trimmed == "CURSOR_POS") return CtlRequest::CursorPos;
    return std::nullopt;
}
...
std::string encodeCtlResponse(const CtlResponse& response)
{
    ...
    if (const auto* cursor = std::get_if<CtlResponseCursorPos>(&response)) {
        return "CURSOR_POS " + std::to_string(cursor->x) + " " + std::to_string(cursor->y) + "\n";
    }
    return {};
}
