...
bool ensureBlitProgram(QOpenGLExtraFunctions* gl, GLuint& program, GLint& texLoc, GLuint& vao)
{
...
bool blitExternalOesToTexture2D(QOpenGLExtraFunctions* gl, unsigned int oesTexture, unsigned int dstFbo, int width, int height)
{
    GLuint program = 0;
    GLint texLoc = 0;
    GLuint vao = 0;
...
WallpaperCaptureItem::WallpaperCaptureItem(QQuickItem* parent) : QQuickItem(parent)
{
...
void WallpaperCaptureItem::replacePendingSource(std::optional<PendingSource> next)
{
...
void WallpaperCaptureItem::stageBuf(quint32 slot, quint32 width, quint32 height, quint32 stride, quint64 modifier, int fd, int syncFd)
{
    m_pendingBufs.push_back(PendingBuf{ slot, width, height, stride, modifier, fd });
...
void WallpaperCaptureItem::stageShm(quint32 width, quint32 height, quint32 stride, int fd)
{
...
std::optional<WallpiperProtocol::MonitorGeometry> WallpaperCaptureItem::currentGeometry() const
{
    auto* win = window();
    if (!win || !win->screen()) {
        return std::nullopt;
...
bool WallpaperCaptureItem::reimportSlot(quint32 slot)
{
    ...
    auto newImage = m_importer.createImageOnly(static_cast<int>(it->second.width), static_cast<int>(it->second.height), it->second.stride, it->second.modifier, dupFd);
    if (!newImage) {
...
void WallpaperCaptureItem::pruneWindow(std::deque<qint64>& timestamps, qint64 nowMsValue, qint64 windowMs)
{
...
int WallpaperCaptureItem::countWithin(const std::deque<qint64>& timestamps, qint64 nowMsValue, qint64 windowMs)
{
    int count = 0;
...
QSGNode* WallpaperCaptureItem::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData*)
{
    ...
    while (!m_pendingBufs.empty()) {
        ...
        auto imported = m_importer.importDmabuf(static_cast<int>(pending.width), static_cast<int>(pending.height), pending.stride, pending.modifier, pending.fd);
        if (!imported) {
        ...
        if (auto* ctx = QOpenGLContext::currentContext()) {
            ...
            gl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, static_cast<int>(pending.width), static_cast<int>(pending.height), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
            ...
            gl->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex.blitTexture, 0);
            gl->glBindFramebuffer(GL_FRAMEBUFFER, 0);

            blitExternalOesToTexture2D(gl, tex.import.texture, tex.blitFbo, static_cast<int>(pending.width), static_cast<int>(pending.height));

            tex.sgTexture.reset(m_importer.wrapTexture2D(win, tex.blitTexture, QSize(static_cast<int>(pending.width), static_cast<int>(pending.height))));
        }
    ...
    if (m_pendingShm) {
        ...
        void* mapped = ::mmap(nullptr, static_cast<size_t>(shm.stride) * shm.height, PROT_READ, MAP_SHARED, shm.fd, 0);
        if (mapped == MAP_FAILED) {
            qWarning() << "[capture] mmap of SHM frame failed";
        } else {
            QImage image(static_cast<const uchar*>(mapped), static_cast<int>(shm.width), static_cast<int>(shm.height), static_cast<int>(shm.stride), QImage::Format_RGB32);
            if (win) {
    ...
    if (m_pendingSource) {
        ...
        m_currentSlot = m_pendingSource->isShm ? std::nullopt : std::make_optional(m_pendingSource->slot);
    ...
    if (m_currentIsShm) {
        ...
        if (it != m_slotTextures.end()) {
            if (haveNewFrame) {
                ...
                if (auto* ctx = QOpenGLContext::currentContext()) {
                    auto* gl = ctx->extraFunctions();
                    blitExternalOesToTexture2D(gl, it->second.import.texture, it->second.blitFbo, static_cast<int>(it->second.width), static_cast<int>(it->second.height));
                }
            }
